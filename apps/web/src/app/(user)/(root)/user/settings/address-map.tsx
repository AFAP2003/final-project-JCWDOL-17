...
export default function AddressMap(props: Props) {
  ...
  const { mutate: fetchLocation, isPending } = useMutation({
    mutationFn: async (param: {
      ...
      // if (isDoingFetch.current) return [];
      // console.log('RUNNN');
  ...
  return (
    <div className="relative border p-2 py-3 rounded-lg shadow-md">
      <h3 className="my-3 mb-5 text-neutral-700 px-3 font-medium">
        Pintpoint Your Address
      </h3>
      ...
      <div className="overflow-hidden rounded-lg relative px-3">
        ...
        {/* Search */}
        <div
          className="absolute right-6 top-3 bg-white w-full max-w-sm overflow-hidden border-[1.8px] rounded-lg shadow-sm group border-green-600"
          style={{ zIndex: 1000 }}
        >
                ...
                {resultSearch.map((loc, idx) => (
                  <div key={idx} className="w-full">
                    <button
                      ...
                      className="text-left text-xs pb-1 hover:bg-green-100 rounded-lg px-2"
                    >
        ...
        {/* Location Result TODO:change places to pinnedLocation */}
        {pinnedLocation && (
          <div className="pt-3 pb-3">
            <p className="font-medium text-neutral-700">
              {pinnedLocation.name}
            </p>
            <p className="text-sm text-neutral-500">{pinnedLocation.address}</p>
          </div>
        )}
      </div>
    </div>
