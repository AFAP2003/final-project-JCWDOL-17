...
class ReportManagementRepository{
        ...
        const items = await prismaclient.orderItem.findMany({
            where:{
                order:{
                    ...
                    paymentStatus:'PAID',
                    status: { notIn: ['WAITING_PAYMENT','WAITING_PAYMENT_CONFIRMATION','CANCELLED'] }
                },
    ...
    async getProductSales(year:number, month:number){
        ...
        const items = await prismaclient.orderItem.findMany({
            where:{
                order:{
                    ...
                    ,paymentStatus:'PAID',
                    status:{
                        notIn:['WAITING_PAYMENT','WAITING_PAYMENT_CONFIRMATION','CANCELLED']
                    }
                }
            }
            ,include:{
        ...
        const totalsByProduct = items.reduce((acc, item) => {
          ...
          .map(([product, { unitsSold, revenue }]) => ({ product, unitsSold, revenue }))
    ...
    async getStockReport(year: number, month: number, storeId: string) {
        ...
        function accumulate(journals: typeof periodJournals) {
          return journals.reduce<Record<string, { added: number; removed: number }>>((acc, j) => {
        ...
        const stockAdded   = periodJournals.filter(j => j.type === 'ADDITION'  )
                                           .reduce((s, j) => s + j.quantity, 0)
        const stockRemoved = periodJournals.filter(j => j.type === 'SUBTRACTION')
                                           .reduce((s, j) => s + j.quantity, 0)
