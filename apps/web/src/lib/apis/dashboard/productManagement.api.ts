    ...
    const [isLoading, setIsLoading] = useState(true);
      ...
      const fetchProducts = async (pageIndex:number,pageSize:number) => {
        ...
        try {
          ...
          const productRes = await fetch(`${API_BASE_URL}/dashboard/products?page=${page}&take=${pageSize}`);
      ...
      const handleCreateProduct = async (values) => {
        try {
          ...
          if (productRes.ok) {
            ...
            console.error(
              'Failed to create Product: Duplicate Product Error',productData
            )
            return false
          } else {
            toast({
      ...
      return { products, isLoading, fetchProducts,handleCreateProduct,handleUpdateProduct,handleDeleteProduct }
